    ...
    if organism:
        ...
        for entry in organisms_database:
            if entry["species"].lower() == organism.lower():
                ...
                config_file["reference_genome"]["id"] = entry["reference_genome"]
                config_file["reference_genome"]["size"] = entry["reference_genome_size"]
                break
        else:
            print(f"Warning: Organism '{organism}' not found in organisms database. AMRFinder and MLST will be run with auto-detection.")

    # Set default path to Bakta database
    config_file["bakta"]["database"] = f"{pipeline_path}/data/baktadb"
